import { NodeData } from '../utils/types.ts';
import { useState, Dispatch, SetStateAction, useEffect } from 'react';

type UseFocusedNodeIndexProps = {
  nodes: NodeData[];
};

export const useFocusedNodeIndex = ({
  nodes
}: UseFocusedNodeIndexProps): [number, Dispatch<SetStateAction<number>>] => {
  const [focusedNodeState, setFocusedNodeIndex] = useState(0);

  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'ArrowUp') {
        setFocusedNodeIndex((index) => Math.max(index - 1, 0));
      }
      if (event.key === 'ArrowDown') {
        setFocusedNodeIndex((index) => Math.max(index + 1, nodes.length - 1));
      }
    };
    document.addEventListener('keydown', onKeyDown);

    return () => document.removeEventListener('keydown', onKeyDown);
  }, [nodes]);

  return [focusedNodeState, setFocusedNodeIndex];
};
